...
# Realny stosunek klas 0:1 (ok. 200:1)
IMBALANCE_RATIO = 200.5 
...
def run_final_audit():
    ...
    for d_name, d_path in DATASETS.items():
        ...
        # SZTYWNY SPLIT 80/20 DLA CAŁOŚCI
        ...
        models = {
            ...
            "CatBoost (Balanced)": CatBoostClassifier(iterations=300, scale_pos_weight=IMBALANCE_RATIO, random_state=42, verbose=0)
        }

        print(f"Audytowanie: {d_name}...")
    ...
    print("\n" + "="*40 + " FINAL PERFORMANCE MATRIX (80/20 SPLIT) " + "="*40)
    report_df = pd.DataFrame(results_table)
    print(report_df.to_string(index=False))
    
    # AUDYT STABILNOŚCI DLA CATBOOST (NAJLEPSZE DANE)
    ...
    cv_scores = cross_val_score(CatBoostClassifier(iterations=100, scale_pos_weight=IMBALANCE_RATIO, verbose=0), X_best, y_best, cv=5)
    print(f"Model: CatBoost na Hybrid S-BERT")
