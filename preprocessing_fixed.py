...
def clean_text_to_set(text):
    """basic normalization: lowercase, remove punctuation, tokenize by whitespace."""
    if not isinstance(text, str) or pd.isna(text): 
        return set()
    # rm non-alphanumeric characters and lowercase
    text = re.sub(r'[^\w\s]', ' ', text.lower())
    # return as a set of unique words (tokens)
...
def run_basic_preprocessing():
    ...
    print(f"\n4/4 Done! Dataset saved to: {OUTPUT_FILE}")
